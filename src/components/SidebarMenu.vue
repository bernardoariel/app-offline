...
const dependenciaActual =
  dependencia.value!['nombre'] || dependencia.value!['dependenciaLetra'];
const closeSidebar = () => {
  visible.value = false;
};
...
const groupedLinks = computed(() => {
  ...
  sidebarLinks
    .filter((link) => link.roles.includes('Oficial Sumariante'))
    .forEach((link) => {
...
<template>
  <div class="card flex justify-content-center">
      ...
      <template #container="{ closeCallback }">
        <div class="flex flex-column h-full">
            ...
            <div v-for="(links, groupName) in groupedLinks" :key="groupName">
              ...
              >
                ...
                <i
                  class="pi"
                  :class="
                    linkState[groupName] ? 'pi-chevron-up' : 'pi-chevron-down'
                  "
                ></i>
              </div>
              <ul
                v-show="linkState[groupName]"
                class="list-none p-3 m-0 overflow-hidden transition-all transition-duration-300"
              >
                <li v-for="link in links" :key="link.pathName">
                  ...
                  <transition name="slidedown">
                    <ul
                      v-show="linkState[link.pathName]"
                      class="list-none pl-4 transition-all transition-duration-300"
                    >
                      <li
          ...
          <div class="mt-auto">
            ...
            <a
              v-ripple
              class="m-3 flex align-items-center p-3 gap-2 border-round text-700 transition-duration-150 transition-colors p-ripple"
            >
    ...
    <Button :severity="colorIcono" text rounded @click="visible = true">
      <template #icon>
        <i :class="['pi', icono]" style="font-size: 1.5rem"></i>
      </template>
    </Button>
  </div>
</template>
