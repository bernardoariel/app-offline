...
import {
  ...
  MyTextArea,
  MyInputMask,
} from '@/components/elementos/index';
...
const validationSchema = yup.object({
  ...
  email: yup
    ...
    .when('tipoDenuncianteSelect.name', {
      is: (value: string) =>
        ['Denunciante', 'Denunciante y Damnificado'].includes(value),
      then: (schema) =>
        schema.required(
          'El correo es obligatorio para Afectados tipo Denunciante'
        ),
...
const { defineField, values, errors, resetForm, meta } = useForm({
  validationSchema,
        ...
        <div class="grid" v-if="!isOrdenPublico">
          ...
          <div class="col-12 flex-wrap flex align-items-end">
            ...
            <div class="md:col-3 col-6">
              ...
              <span
                class="text-red-400"
                v-if="errors.sexoSelect ? true : false"
              >
                {{ errors.sexoSelect }}
              </span>
            </div>
