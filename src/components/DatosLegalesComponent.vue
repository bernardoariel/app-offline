...
const handleDropdownChange = (
  ...
  } else if (campo === 'selectYear') {
    selectedYear.value = newValue.value;
  }
};
  ...
  <div class="grid">
    <div class="xl:col-9 col-12">
      ...
      <MyInputNumber
        ...
        v-bind="legajoAttrs"
        :error="errors.legajo"
        @update:model-value="(event) => handleInputChange('nroLegajo', +event)"
        color
      />
    </div>
    ...
    >
      ...
      >
        <label :for="item" class="capitalize">{{
          separateCamelCase(
            item === 'juzgadoInterviniente' ? 'Interviniente' : item
          )
        }}</label>
        <MyDropdown
