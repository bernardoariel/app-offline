...
import type { DatosLegalesDiligencia } from "@/interfaces/datosLegalesForm.interface";
...
const useDatosDiligencia = (actuacion: ref<string>) => {
  ...
  const articulosRelacionados = [
    "Articulo 65 ",
    "Ley N\u00ba 941-R",
    "Art 192 aprovechamiento malicioso de credito",
  ];
  ...
  const getIdToFind = (actuacion: string) => {
    ...
    if (
      actuacion.startsWith("ufi-delitos-especiales-") &&
      actuacion !== "ufi-delitos-especiales-paradero"
    ) {
    ...
    if (
      actuacion.startsWith("ufi") &&
      ufiOficio.some((term) => actuacion.includes(term))
    ) {
      return "ufi-oficio";
    }
  ...
  const processedAfectados = computed(() => {
    return afectados.value
      .map((a, index) => {
        if (a.descripcionOrdenPublico) return ``;
        const isLast = index === afectados.value.length - 1;
        const separator = isLast ? "." : ",";
        const apellidoNombre =
          getUpperCase(a.apellido) + ", " + getTitleCase(a.nombre);
        const documento =
          a.typeDocumento && a.nroDocumento
            ? "con " + a.typeDocumento + " Nº " + String(a.nroDocumento)
            : "";
        ...
        return `${getStyle(apellidoNombre)} ${getStyle(documento)} ${getStyle(
          nacionalidad
        )} ${getStyle(edad)} ${getStyle(instruccion)} ${getStyle(
          profesion
        )} ${getStyle(domicilio, isLast)}`;
      })
      .join(" ");
  });

  const processedVinculados = computed(() => {
    return vinculados.value
      .map((v, index) => {
        if (v.descripcionDesconocido) return ``;
        const isLast = index === vinculados.value.length - 1;
        const separator = isLast ? "." : ",";
        const apellidoNombre =
          getUpperCase(v.apellido) + ", " + getTitleCase(v.nombre);
        const documento =
          v.typeDocumento && v.nroDocumento
            ? "con " + v.typeDocumento + " Nº " + String(v.nroDocumento)
            : "";
  ...
  const processedIntervinientes = computed(() => {
    return intervinientes.value
      .map((item, index) => {
        const isLast = index === intervinientes.value.length - 1;
        const separator = isLast ? "." : ",";

        return `${getStyle(
          getTitleCase(item.jerarquia) +
            " " +
            getUpperCase(item.apellido) +
            " " +
            getTitleCase(item.nombre)
        )}
          ${getStyle(
            " adscripto/s a numerario de  " + item.dependencia
          )}${separator}`;
      })
      .join(" ");
  });
  const processedEfectos = computed(() => {
    return efectos.value
      .map((e, index) => {
        ...
        const textoSecuestro =
          estado.toLowerCase() === "secuestrado" ? "el secuestro de " : "";
  ...
  const processedText = computed<ProcessedText>(() => {
    ...
    if (diligenciaSeleccionada.value) {
      header = diligenciaSeleccionada.value.header
        .replace(
          "@dependencia",
          getStyle(dependenciaDataLocal.dependencia.nombre, "blue-600")
        )
        .replace(
          "@departamento",
          getStyle(dependenciaDataLocal.dependencia.departamento)
        )
        .replace(
          "@fechaactuacion",
          getStyle(
      ...
      footer = diligenciaSeleccionada.value.footer
        ...
        .replace(
          "@ayudanteFiscal",
          getStyle(processedAyudanteFiscal.value, "blue-600")
        )
        .replace("@fiscal", getStyle(processedFiscal.value, "blue-600"))
        .replace(
          "@articulosRelacionados",
          getStyle(processedArticulosRelacionados.value, "blue-600")
        );
  ...
  const processedFechaUbicacion = computed(() => {
    ...
    if (
      !ubicacionItem ||
      !ubicacionItem.desdeFechaHora ||
      !ubicacionItem.calle ||
      !ubicacionItem.numero
    ) {
      return "";
    }
    ...
    const ubicacion = `se encontraban de recorridas por Calle ${getTitleCase(
      ubicacionItem.calle
    )} Número: ${ubicacionItem.numero}`;
  ...
  const headerTextComputed = computed({
    get() {
      return isEditingHeader.value
        ? headerContainer.value
        : processedHeaderText.value;
    },
    set(newValue) {
  ...
  const footerTextComputed = computed({
    get() {
      return isEditingFooter.value
        ? footerContainer.value
        : processedFooterText.value;
    },
    set(newValue) {
