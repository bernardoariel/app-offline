...
const useDatosLegales = () => {
  ...
  const fakeValues = {
    nroLegajo: generateRandomNumber(),
  ...
  const addDataFake = () => {
    ...
    selectedJuzgadoInterviniente.value =
      fakeValues.selectedJuzgadoInterviniente;
    selectedArticulosRelacionados.value =
      fakeValues.selectedArticulosRelacionados;
  };
  ...
  const setData = (data: { datosLegales: string }) => {
    ...
    selectedYear.value = datosLegales.selectYear
      ? { name: datosLegales.selectYear }
      : null;
    selectedSitio.value = datosLegales.selectSitio
      ? { name: datosLegales.selectSitio }
      : null;
    selectedDelito.value = datosLegales.selectDelito
      ? { name: datosLegales.selectDelito }
      : null;
    selectedUfiNro.value = datosLegales.selectUfiNro
      ? { name: datosLegales.selectUfiNro }
      : null;
    selectedAyudanteFiscal.value = datosLegales.selectAyudanteFiscal
      ? { name: datosLegales.selectAyudanteFiscal }
      : null;
    selectedFiscalCargo.value = datosLegales.selectFiscalCargo
      ? { name: datosLegales.selectFiscalCargo }
      : null;
    selectedModusOperandi.value = datosLegales.selectModusOperandi
      ? { name: datosLegales.selectModusOperandi }
      : null;
    selectedCausaCaratula.value = datosLegales.selectCausaCaratula
      ? { name: datosLegales.selectCausaCaratula }
      : null;
    selectedJuzgadoInterviniente.value = datosLegales.selectJuzgadoInterviniente
      ? { name: datosLegales.selectJuzgadoInterviniente }
      : null;
    itemsCausaCaratula.value = datosLegales.opcionesCausaCaratula
      ? datosLegales.opcionesCausaCaratula.map((name) => ({ name }))
      : [];
    itemsArticulosRelacionados.value =
      datosLegales.opcionesArticulosRelacionados
        ? datosLegales.opcionesArticulosRelacionados.map((name) => ({ name }))
        : [];
    nombreActuacion.value =
      datosLegales.nombreActuacion || "Sumario por denuncia";
    selectedArticulosRelacionados.value =
      datosLegales.selectedArticulosRelacionados
        ? { name: datosLegales.selectedArticulosRelacionados }
        : null;
  };
  return {
    selectedYear,
